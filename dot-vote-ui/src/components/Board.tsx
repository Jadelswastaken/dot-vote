...
function SkeletonCard() {
  return (
    <div className="break-inside-avoid inline-block w-full p-4 shadow-block-static transition-all bg-grey animate-pulse">
      <div className="h-4 bg-text w-3/4 mb-3" />
      <div className="h-3 bg-text w-full mb-2" />
      <div className="h-3 bg-text w-2/3 mb-4" />
      <div className="border-t border-text pt-2 flex justify-between items-center">
        <div className="flex gap-2">
          <div className="w-6 h-6 rounded-full bg-text" />
          <div className="w-6 h-6 rounded-full bg-text" />
        </div>
        <div className="h-8 w-16 bg-text" />
      </div>
    </div>
  );
}
...
export default function Board({ isAuthenticated, username }: BoardProps) {
  ...
  // Refetch when the sort changes or auth state flips. Refetching on
  // isAuthenticated keeps user_has_voted in sync with the server: votes appear
  // on login and disappear on logout without requiring a page refresh.
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    setError('');
        ...
        <div className="flex items-center gap-2 text-md bg-plum px-3 py-1.5 shadow-block-static">
          ...
          <button
            className={`border-none  px-3 py-1 cursor-pointer  ${
              sort === 'popular' ? 'bg-dusty-blue shadow-block-static ' : 'hover:bg-hopbush hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-lg'
            }`}
          ...
          <button
            className={`border-none  px-3 py-1 cursor-pointer  ${
              sort === 'newest' ? 'bg-dusty-blue shadow-block-static ' : 'hover:bg-hopbush hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-lg'
            }`}
      ...
      {!loading && !error && openIdeas.length === 0 && (
        <p className="text-center py-12 text-sm">
          {isAuthenticated
            ...
            : 'No ideas yet — sign in to add one.'}
        </p>
      )}
